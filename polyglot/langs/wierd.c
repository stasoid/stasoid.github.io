// stasoid: changes:
// * SIZE 128 -> 4096 to handle long lines
// * IF instruction is prohibited because it's very buggy and I don't want to rely on it
...
int condition (struct iplist ** ip)
{
 ...
 if (workspace[list->x-tmpx][list->y-tmpy] != ' ')
	...
	fprintf (stderr,  "IF instruction is prohibited");
	exit(1);
	}
